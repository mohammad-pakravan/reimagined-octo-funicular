...
async def search_users_button(message: Message):
    ...
    async for db_session in get_db():
        ...
        await message.answer(
            "🔍 جستجوی کاربران\n\n"
            "از دکمه‌های زیر استفاده کن تا بر اساس جنسیت، همشهری یا هم‌استانی بودن، مخاطب پیدا کنی.\n"
            "بعد از انتخاب و دیدن نتایج، می‌تونی از همونجا برای طرف مقابل درخواست چت بفرستی یا یک پیام خصوصی ارسال کنی. ✉️",
